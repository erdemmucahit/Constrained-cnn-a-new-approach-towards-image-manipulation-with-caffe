...
if __name__ == '__main__':
	...
	print(ll)
	os.chdir('/home/erdem/test_images_tubitak') #Change work directory to where you saved your Dresden images
	n = 1280 #cropping height 256x5
	m= 1280 # cropping width 256x5
	
	X = np.zeros((1650,1,256,256), dtype=np.uint8) #Initialize image data with zeros 
	y = np.zeros(1650, dtype=np.int64) #Initialize image labels with zeros
	count = 0
	k = 0
	for p in ll:
	...
	os.chdir('/home/erdem/test_lmdb_file2/') #Change the work directory under where you are going to save the test_lmdb data
